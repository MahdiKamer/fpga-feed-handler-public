...
|   └── generate_ltx.sh   # Generate Logic Analyser signal definitions offline
│
...
- Measured as:
  ...
  - ts_rx → ts_tx (from when the header_inserter drives headers into AXI-S / MAC transmit, until the first ITCH payload byte is observed at the ITCH parser).
...
- Wall-clock equivalent:
  ...
  - At 125 MHz fabric clock,
    ...
    - 99 cycles ≈ 792 ns (end-to-end).

    - 12 cycles ≈ 96 ns (parser).
