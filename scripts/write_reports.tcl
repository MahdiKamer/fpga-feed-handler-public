...
if {[file exists $rpt_file]} {
    ...
    if {$WNS ne "N/A" && $WNS < 0} {
        puts $fp "⚠️  Negative WNS: tighten constraints, check critical path placement, consider pipelining."
    } elseif {$TNS ne "N/A" && $TNS < 0} {
        puts $fp "⚠️  Negative TNS: multiple paths failing, review floorplanning or timing exceptions."
    } elseif {$WHS ne "N/A" && $WHS < 0} {
        puts $fp "⚠️  Negative WHS: hold violation, add delay buffers or fix skew."
    } elseif {$THS ne "N/A" && $THS < 0} {
        puts $fp "⚠️  Negative THS: widespread hold issues, check clocking and placement."
    } else {
